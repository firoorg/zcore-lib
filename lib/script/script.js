...
Script.fromBuffer = function(buffer) {
  ...
  while (!br.finished()) {
    try {
      ...
      } else if (opcodenum === Opcode.OP_SIGMASPEND) {
        len = br.buf.length - br.pos;
        buf = br.read(len);
        script.chunks.push({
          buf: buf,
          len: len,
...
Script.prototype.isExPublicKeyHashOut = function() {
  return !!(this.chunks.length === 6 &&
      this.chunks[0].opcodenum === Opcode.OP_EXCHANGEADDR &&
      this.chunks[1].opcodenum === Opcode.OP_DUP &&
      this.chunks[2].opcodenum === Opcode.OP_HASH160 &&
      this.chunks[3].buf &&
      this.chunks[3].buf.length === 20 &&
      this.chunks[4].opcodenum === Opcode.OP_EQUALVERIFY &&
      this.chunks[5].opcodenum === Opcode.OP_CHECKSIG);
};
...
Script.prototype.getExPublicKeyHash = function() {
  $.checkState(this.isExPublicKeyHashOut(), 'Can\'t retrieve ExPublicKeyHash from a non-PKH output');
...
Script.prototype.getData = function() {
  ...
  if (this.isExPublicKeyHashOut()) {
    return Buffer.from(this.chunks[3].buf);
  }
...
Script.outputIdentifiers.EXPUBKEYHASH_OUT = Script.prototype.isExPublicKeyHashOut;
...
/**
 * @returns {Script} a new pay to Exhange public key hash output for the given
 * address or public key
 * @param {(Address|PublicKey)} to - destination address or public key
 */
Script.buildExPublicKeyHashOut = function(to) {
  $.checkArgument(!_.isUndefined(to));
  $.checkArgument(to instanceof PublicKey || to instanceof Address || _.isString(to));
  if (to instanceof PublicKey) {
    to = to.toAddress();
...
Script.fromAddress = function(address) {
  ...
  } else if (address.isPayToExPublicKeyHash()) {
    return Script.buildExPublicKeyHashOut(address);
  }
...
Script.prototype._getOutputAddressInfo = function() {
  ...
  } else if (this.isExPublicKeyHashOut()) {
    info.hashBuffer = this.getData();
    info.type = Address.PayToExPublicKeyHash;
  } else {
    return false;
  }
  return info;
